...
VVIObjF::VVIObjF(float kappa) : mode_(1) {
  ...
  float invKappa = 1.f / kappa;
  h_[4] = 1.f - 0.42278433999999998f + (7.6f * invKappa);
  h4 = -(7.6f * invKappa) - (0.57721566f + 1.f);
  h5 = vdt::fast_logf(kappa);
  h6 = invKappa;
  ...
  ul = lq - 6.5f;
  auto f2 = [h_](float x) { return h_[4] - x + 1.f * (vdt::fast_logf(std::abs(x)) + VVIObjFDetails::expint(x)); };
  VVIObjFDetails::dzero(ll, ul, u, rv, 1.e-3f, 100, f2);
  q = 1. / u;
  t1_ = h4 * q - h5 - (q + 1.f) * (vdt::fast_logf((fabs(u))) + VVIObjFDetails::expint(u)) + vdt::fast_expf(-u) * q;
  t_ = t1_ - t0_;
  omega_ = 6.2831853000000004f / t_;
  h_[0] = kappa * (0.57721566f + 2.f) + 9.9166128600000008f;
  if (kappa >= .07) {
    h_[0] += 6.90775527f;
  }
  h_[1] = kappa;
  ...
  VVIObjFDetails::dzero(5.f, 155.f, x0_, rv, 1.e-3f, 100, f1);
  n = x0_ + 1.;
  d = vdt::fast_expf(kappa * ((.57721566f - h5) + 1.f)) * .31830988654751274f;
  a_[n - 1] = 0.f;
  q = -1.;
  q2 = 2.;
  for (k = 1; k < n; ++k) {
